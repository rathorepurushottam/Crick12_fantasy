...
export const modifyName = (name: string) => {
  ...
  if (nameParts?.length >= 3) {
    ...
    return modifiedName?.length == 9
      ? modifiedName
      : modifiedName?.length > 9
      ? modifiedName?.slice(0, 7) + '..'
      : modifiedName;
...
export const formatDate = (dateString: string | number | Date) => {
  ...
  const isYesterday =
    ...
    new Date(currentDate - 86400000).toDateString(); // 86400000 milliseconds in a day
  ...
  if (isToday) {
    ...
    return (
      inputDate.toLocaleDateString('en-US', {month: 'short', day: 'numeric'}) +
      ' ' +
...
export const transformData = (originalData: any[]) => {
  return (
    ...
    originalData
      ?.map(
        (contest: {name: any; winning_amount: any; more: any; data: any}) => {
          ...
          const transformedContestData = data
            ...
            .sort(
              (a: {winning_amount: number}, b: {winning_amount: number}) =>
                b.winning_amount - a.winning_amount,
            ); // Sort by winning_amount in descending order
...
export const playerRollImageTwo = (
  array: {numberid: string; profile_image: any; playing_role: string},
  arrayTwo: {numberid: string},
) => {
